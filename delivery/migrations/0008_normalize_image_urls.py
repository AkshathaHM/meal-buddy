...
def decode_image_urls(apps, schema_editor):
    ...
    for model in (restaurant_model, item_model):
        for record in model.objects.exclude(picture='').only('id', 'picture'):
            if r'\u' not in record.picture:
                continue

            record.picture = re.sub(
                r'\\u([0-9a-fA-F]{4})',
...
class Migration(migrations.Migration):
    dependencies = [
        ('delivery', '0007_item_image'),
    ]

    operations = [
        migrations.RunPython(decode_image_urls, migrations.RunPython.noop),
    ]